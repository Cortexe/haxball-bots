...
const roomName = "⚽ Futsal Portugal - BOT ⚽";
const botName = "Ambrosio";
const maxPlayers = 12;
const roomPublic = true;
const geo = [{"code": "PT", "lat": 41.5, "lon": -8.6}];
...
function checkTime() {
	...
	if (Math.abs(scores.time - scores.timeLimit) <= 0.01 && scores.timeLimit != 0) {
		...
		goldenGoal = true;
		room.sendAnnouncement("⚽ O próximo a marcar ganha!");
	}
	if (Math.abs(drawTimeLimit * 60 - scores.time - 60) <= 0.01 && players.length > 2) {
		if (checkTimeVariable == false) {
			checkTimeVariable = true;
			setTimeout(() => { checkTimeVariable = false; }, 10);
			room.sendAnnouncement("⌛ Faltam 60 segundos!");
		}
	}
...
function endGame(winner) { // handles the end of a game : no stopGame function inside
	...
	if (winner == Team.RED) {
		streak++;
		room.sendAnnouncement("🔴 Equipa Vermelha Ganhou por " + scores.red + "-" + scores.blue + "! Vitórias consecutivas: " + streak + " 🏆");
	}
	else if (winner == Team.BLUE) {
		streak = 1;
		room.sendAnnouncement("🔵 Equipa Azul Ganhou por " + scores.blue + "-" + scores.red + "! Vitórias consecutivas: " + streak + " 🏆");
	}
	else {
		streak = 0;
		room.sendAnnouncement("💤 O jogo termina como empate 💤");
    }
    room.sendAnnouncement("⭐ Posse de Bola : 🔴 " + (Rposs*100).toPrecision(3).toString() + "% : " + (Bposs*100).toPrecision(3).toString() + "% 🔵");
    scores.red == 0 ? (scores.blue == 0 ? room.sendAnnouncement("🏆 " + GKList[0].name + " e " + GKList[1].name + " nao sofreram golos! ") : room.sendAnnouncement("🏆 " + GKList[1].name + " não sofreu golos! ")) : scores.blue == 0 ? room.sendAnnouncement("🏆 " + GKList[0].name + " não sofreu golos! ") : null;
	updateStats();
}
...
function activateChooseMode() {
	inChooseMode = true;
	slowMode = 2;
	room.sendAnnouncement("Modo lento ativado por 2 segundos!");
}
...
function deactivateChooseMode() {
	...
	if (slowMode != 0) {
		slowMode = 0;
		room.sendAnnouncement("O modo lento terminou.");
	}
	redCaptainChoice = "";
	blueCaptainChoice = "";
}
...
function handleInactivity() { // handles inactivity : players will be kicked after afkLimit
	...
	for (var i = 0; i < extendedP.length ; i++) {
		if (extendedP[i][eP.ACT] == 60 * (2/3 * afkLimit)) {
			room.sendAnnouncement("❗️❕❗️ @" + room.getPlayer(extendedP[i][eP.ID]).name + ", se não te mexeres nos próximos " + Math.floor(afkLimit / 3) + " segundos, vais ser kickado ❗️❕❗️", extendedP[i][eP.ID]);
		}
		if (extendedP[i][eP.ACT] >= 60 * afkLimit) {
			...
			room.kickPlayer(extendedP[i][eP.ID], "Ficaste AFK demasiado tempo", false);
		}
	}
}
...
function updateRoleOnPlayerOut() {
		...
		if (players.length >= 2 * maxTeamSize && scores.time >= (5/6) * game.scores.timeLimit && teamR.length != teamB.length) {
			if (teamR.length < teamB.length) {
				if (scores.blue - scores.red == 2) {
					endGame(Team.BLUE);
					room.sendAnnouncement("🤖 Ragequit detetado. Jogo terminado 🤖");
	...
	if (inChooseMode) {
		...
		if (Math.abs(teamR.length - teamB.length) == teamS.length) {
			room.sendAnnouncement("🤖 Sem escolhas restantes, deixa-me tratar da situação... 🤖");
		...
		if (streak == 0 && room.getScores() == null) {
			if (Math.abs(teamR.length - teamB.length) == 2) { // if someone left a team has 2 more players than the other one, put the last chosen guy back in his place so it's fair
				room.sendAnnouncement("🤖 A equilibrar as equipas... 🤖");
...
function choosePlayer() {
	...
	if (teamR.length <= teamB.length && teamR.length != 0) {
		room.sendAnnouncement("➡️ Para escolheres um jogador, escreve no chat a posição onde ele está ou escreve 'top', 'random' or 'bottom'", teamR[0].id);
		timeOutCap = setTimeout(function (player) { room.sendAnnouncement("➡️ Despacha-te @" + player.name + ", só tens mais " + Number.parseInt(chooseTime / 2) + " segundos para escolher!", player.id); timeOutCap = setTimeout(function (player) { room.kickPlayer(player.id, "You didn't choose in time !", false); }, chooseTime * 500, teamR[0]); }, chooseTime * 1000, teamR[0]);
	}
	else if (teamB.length < teamR.length && teamB.length != 0) {
		room.sendAnnouncement("➡️ Para escolheres um jogador, escreve no chat a posição onde ele está ou escreve 'top', 'random' or 'bottom'", teamB[0].id);
...
setInterval(() => {
	...
	if (statNumber % 5 == 2) {
		...
		tableau.sort(function (a, b) { return b[1] - a[1]; });
		room.sendAnnouncement("Golos> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1]);
	}
	if (statNumber % 5 == 3) {
		...
		tableau.sort(function (a, b) { return b[1] - a[1]; });
		room.sendAnnouncement("Assistências> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1]);
	}
	if (statNumber % 5 == 4) {
		...
		tableau.sort(function (a, b) { return b[1] - a[1]; });
		room.sendAnnouncement("Clean Sheet> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1]);
	}
	statNumber++;
...
room.onPlayerJoin = function(player) {
	...
	if (localStorage.getItem(player.auth) != null) {
		if (JSON.parse(localStorage.getItem(player.auth))[Ss.RL] != "player") {
			...
			room.sendAnnouncement((JSON.parse(localStorage.getItem(player.auth))[Ss.RL] == "master" ? "Master " : "Admin ") + player.name + " has connected to the room !");
		}
    }
    setTimeout(() => {
        room.sendAnnouncement("Aviso: O Bot está em fase de testes. Podem haver alguns erros", player.id);
    }, 2000);
}
...
room.onPlayerChat = function (player, message) {
	...
	if (["!help", "!ajuda"].includes(message[0].toLowerCase())) {
        console.log("Help");
		room.sendAnnouncement("➡️ Comandos: !eu, !jogos, !vitorias, !golos, !assistencias, !cs, !afks, !mutes, !bans.", player.id);
		player.admin ? room.sendAnnouncement("➡️ Admin : !mute <duration = 3> #<id>, !unmute all/#<id>, !clearbans <number = all>, !slow <duration>, !endslow", player.id) : null;
	}else if (["!ids"].includes(message[0].toLowerCase())) {
        let ids = "| ";
        players.forEach(element => {
            ids = ids + element.name + ":" + element.id + " | ";
        });
        room.sendAnnouncement(ids, player.id);
    }
		...
		if (players.length != 1 && player.team != Team.SPECTATORS) {
			if (player.team == Team.RED && streak > 0 && room.getScores() == null) {
				...
				room.sendAnnouncement("Nao podes ir AFK enquanto estas numa equipa!", player.id);
				return false;
			}
		}
		...
		room.sendAnnouncement(player.name + (getAFK(player) ? " está AFK!" : " já não AFK!"));
		...
		if (cstm == "➡️ Jogadores AFK: ") {
			room.sendAnnouncement("➡️ Não existe ninguém AFK!", player.id);
			return false;
		}
		cstm = cstm.substring(0, cstm.length - 2);
		cstm += ".";
	...
	else if (["!me", "!eu"].includes(message[0].toLowerCase())) {
		var stats;
		localStorage.getItem(getAuth(player)) ? stats = JSON.parse(localStorage.getItem(getAuth(player))) : stats = [0, 0, 0, 0, "0.00", 0, 0, 0, 0, "0.00"];
		room.sendAnnouncement("➡️ " + player.name + "> Jogos: " + stats[Ss.GA] + ", Vitórias: " + stats[Ss.WI] + ", Empates: " + stats[Ss.DR] + ", Derrotas: " + stats[Ss.LS] + ", WR: " + stats[Ss.WR] + "%, Golos: " + stats[Ss.GL] + ", Assistências: " + stats[Ss.AS] + ", GK: " + stats[Ss.GK] + ", CS: " + stats[Ss.CS] + ", CS%: " + stats[Ss.CP] + "%", player.id);
	}
	else if (["!games", "!jogos"].includes(message[0].toLowerCase())) {
		var tableau = [];
		Object.keys(localStorage).forEach(function (key) { if (!["player_name", "view_mode", "geo", "avatar", "player_auth_key"].includes(key)) { tableau.push([(JSON.parse(localStorage.getItem(key))[Ss.NK]), (JSON.parse(localStorage.getItem(key))[Ss.GA])]); } });
		if (tableau.length < 5) {
			room.sendAnnouncement("➡️ Nao foram jogados jogos suficientes.", player.id);
			return false;
		}
		tableau.sort(function (a, b) { return b[1] - a[1]; });
		room.sendAnnouncement("➡️ Jogos> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1], player.id);
	}
	else if (["!wins",  "!vitorias"].includes(message[0].toLowerCase())) {
		var tableau = [];
		Object.keys(localStorage).forEach(function (key) { if (!["player_name", "view_mode", "geo", "avatar", "player_auth_key"].includes(key)) { tableau.push([(JSON.parse(localStorage.getItem(key))[Ss.NK]), (JSON.parse(localStorage.getItem(key))[Ss.WI])]); } });
		if (tableau.length < 5) {
			room.sendAnnouncement("➡️ Nao foram jogados jogos suficientes.", player.id);
		...
		tableau.sort(function (a, b) { return b[1] - a[1]; });
		room.sendAnnouncement("➡️ Vitórias> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1], player.id);
	}
	else if (["!goals",  "!golos"].includes(message[0].toLowerCase())) {
		var tableau = [];
		Object.keys(localStorage).forEach(function (key) { if (!["player_name", "view_mode", "geo", "avatar", "player_auth_key"].includes(key)) { tableau.push([(JSON.parse(localStorage.getItem(key))[Ss.NK]), (JSON.parse(localStorage.getItem(key))[Ss.GL])]); } });
		if (tableau.length < 5) {
			room.sendAnnouncement("➡️ Nao foram jogados jogos suficientes.", player.id);
		...
		tableau.sort(function (a, b) { return b[1] - a[1]; });
		room.sendAnnouncement("➡️ Golos> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1], player.id);
	}
	else if (["!assists",  "!assistencias"].includes(message[0].toLowerCase())) {
		var tableau = [];
		Object.keys(localStorage).forEach(function (key) { if (!["player_name", "view_mode", "geo", "avatar", "player_auth_key"].includes(key)) { tableau.push([(JSON.parse(localStorage.getItem(key))[Ss.NK]), (JSON.parse(localStorage.getItem(key))[Ss.AS])]); } });
		if (tableau.length < 5) {
			room.sendAnnouncement("➡️ Nao foram jogados jogos suficientes.", player.id);
		...
		tableau.sort(function (a, b) { return b[1] - a[1]; });
		room.sendAnnouncement("➡️ Assistências> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1], player.id);
	}
		...
		tableau.sort(function (a, b) { return b[1] - a[1]; });
		room.sendAnnouncement("➡️ CS> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1], player.id);
	}
		...
		if (message[1] == adminPassword) {
			...
			if (stats[Ss.RL] != "master") {
				stats[Ss.RL] = "master";
				room.sendAnnouncement(player.name + " é agora master da sala!");
			...
			if (message.length >= 2 && message[1][0] == "#") {
				...
				if (!Number.isNaN(Number.parseInt(message[1])) && room.getPlayer(Number.parseInt(message[1])) != null) {
					...
					if (stats[Ss.RL] == "player") {
						...
						room.sendAnnouncement(room.getPlayer(Number.parseInt(message[1])).name + " é agora Administrador!");
					}
				}
			}
		}
			...
			if (message.length >= 2 && message[1][0] == "#") {
				...
				if (!Number.isNaN(Number.parseInt(message[1])) && room.getPlayer(Number.parseInt(message[1])) != null) {
					...
					if (stats[Ss.RL] == "admin") {
						room.sendAnnouncement(room.getPlayer(Number.parseInt(message[1])).name + " não é mais Administrador da sala!");
						stats[Ss.RL] = "player";
						localStorage.setItem(getAuth(room.getPlayer(Number.parseInt(message[1]))), JSON.stringify(stats));
		...
		var cstm = "➡️ Jogadores Silenciados : ";
		for (var i = 0; i < extendedP.length; i++) {
		...
		if (cstm == "➡️ Jogadores Silenciados: ") {
			room.sendAnnouncement("➡️ Não existe nenhum jogador silenciado!", player.id);
			return false;
		}
		cstm = cstm.substring(0, cstm.length - 2);
		cstm += ".";
			...
			if (!Number.isNaN(Number.parseInt(message[1])) && message.length > 1) {
				...
				if (message[2].length > 1 && message[2][0] == "#") {
					...
					if (!Number.isNaN(Number.parseInt(message[2])) && room.getPlayer(Number.parseInt(message[2])) != null) {
						...
						room.sendAnnouncement(room.getPlayer(Number.parseInt(message[2])).name + " foi silenciado por " + (timeOut / 60000) + " minutos 🤐");
					}
				}
			}
				...
				if (message[1].length > 1 && message[1][0] == "#") {
					...
					if (!Number.isNaN(Number.parseInt(message[1])) && room.getPlayer(Number.parseInt(message[1])) != null) {
						...
						room.sendAnnouncement(room.getPlayer(Number.parseInt(message[1])).name + " foi silenciado por 3 minutos 🤐");
					}
				}
			}
		}
		...
		if (player.admin && message.length >= 2) {
			if (message[1] == "all") {
				...
				room.sendAnnouncement("Lista de silenciados limpa.");
			}
			else if (!Number.isNaN(Number.parseInt(message[1])) && room.getPlayer(Number.parseInt(message[1])) != null && getMute(room.getPlayer(Number.parseInt(message[1])))) {
				setMute(room.getPlayer(Number.parseInt(message[1])), false);
				room.sendAnnouncement(room.getPlayer(Number.parseInt(message[1])).name + " já pode falar!");
			}
			else if (Number.isNaN(Number.parseInt(message[1]))) {
				if (message[1].length > 1 && message[1][0] == "#") {
					message[1] = message[1].substring(1, message[1].length);
	...
	else if (["!slow",  "!lento"].includes(message[0].toLowerCase())) {
		if (player.admin) {
			if (message.length == 1) {
				slowMode = 2;
				room.sendAnnouncement("Modo lento ativado por 2 segundos!");
				...
				if (!Number.isNaN(Number.parseInt(message[1]))) {
					if (Number.parseInt(message[1]) > 0) {
						...
						room.sendAnnouncement("Modo lento ativado por " + slowMode + " segundos!");
						return false;
					}
				}
				slowMode = 2;
	...
	else if (["!endslow",  "!acabarlento"].includes(message[0].toLowerCase())) {
		if (player.admin) {
			slowMode != 0 ? room.sendAnnouncement("O modo lento teminou.") : null;
			slowMode = 0;
		}
	}
	else if (["!banlist", "!bans"].includes(message[0].toLowerCase())) {
		if (banList.length == 0) {
			room.sendAnnouncement("➡️ Nao existe ninguém banido!", player.id);
			return false;
		}
		var cstm = "➡️ Jogadores Banidos: ";
		...
		if (player.admin) {
			if (message.length == 1) {
				room.clearBans();
				room.sendAnnouncement("Lista de Bans limpa!");
				banList = [];
			}
			if (message.length == 2) {
				if (!Number.isNaN(Number.parseInt(message[1]))) {
					if (Number.parseInt(message[1]) > 0) {
						...
						if (banList.length != banList.filter((array) => array[1] != ID)) {
							room.sendAnnouncement(banList.filter((array) => array[1] == ID)[0][0] + " foi desbanido!");
						}
	...
	else if (["!bb", "!bye", "!cya", "!gn",  "!fui", "!xau"].includes(message[0].toLowerCase())) {
		room.kickPlayer(player.id, "Xau!", false);
	}
		...
		if (player.id == teamR[0].id || player.id == teamB[0].id) { // we care if it's one of the captains choosing
			if (teamR.length <= teamB.length && player.id == teamR[0].id) { // we care if it's red turn && red cap talking
				if (["top", "auto"].includes(message[0].toLowerCase())) {
					...
					clearTimeout(timeOutCap);
					room.sendAnnouncement(player.name + " escolheu o primeiro!");
					return false;
				}
					...
					clearTimeout(timeOutCap);
					room.sendAnnouncement(player.name + " escolheu aleatoriamente!");
					return false;
				}
					...
					clearTimeout(timeOutCap);
					room.sendAnnouncement(player.name + " escolheu o último!");
					return false;
				}
				else if (!Number.isNaN(Number.parseInt(message[0]))) {
					if (Number.parseInt(message[0]) > teamS.length || Number.parseInt(message[0]) < 1) {
						room.sendAnnouncement("➡️ O teu número é inválido!", player.id);
						return false;
					}
					else {
						room.setPlayerTeam(teamS[Number.parseInt(message[0]) - 1].id, Team.RED);
						room.sendAnnouncement(player.name + " escolheu " + teamS[Number.parseInt(message[0]) - 1].name);
						return false;
					}
				}
			}
	...
	if (getMute(player)) {
		room.sendAnnouncement("Estás silenciado, não podes falar no chat 🤫", player.id);
		return false;
	}
	if (slowMode > 0) {
		if (!player.admin) {
...
room.onTeamGoal = function(team) {
	...
	if (lastPlayersTouched[0] != null && lastPlayersTouched[0].team == team) {
		if (lastPlayersTouched[1] != null && lastPlayersTouched[1].team == team) {
			room.sendAnnouncement("⚽ " + getTime(scores) + " Golo de " + lastPlayersTouched[0].name + " ! Assistido por " + lastPlayersTouched[1].name + ". Velocidade: " + ballSpeed.toPrecision(3).toString() + "km/h " + (team == Team.RED ? "🔴" : "🔵"));
			game.goals.push(new Goal(scores.time, team, lastPlayersTouched[0], lastPlayersTouched[1]));
		}
		else {
			room.sendAnnouncement("⚽ " + getTime(scores) + " Golo de " + lastPlayersTouched[0].name + " ! Velocidade: " + ballSpeed.toPrecision(3).toString() + "km/h " + (team == Team.RED ? "🔴" : "🔵"));
			game.goals.push(new Goal(scores.time, team, lastPlayersTouched[0], null));
		}
	}
	else {
		room.sendAnnouncement("😂 " + getTime(scores) + " Auto golo de " + lastPlayersTouched[0].name + " ! Velocidade: " + ballSpeed.toPrecision(3).toString() + "km/h " + (team == Team.RED ? "🔴" : "🔵"));
...
room.onPlayerAdminChange = function (changedPlayer, byPlayer) {
	if (getMute(changedPlayer) && changedPlayer.admin) {
		room.sendAnnouncement(changedPlayer.name + " já pode falar no chat.");
		setMute(changedPlayer, false);
	}
	if (byPlayer && byPlayer.id != 0 && localStorage.getItem(getAuth(byPlayer)) && JSON.parse(localStorage.getItem(getAuth(byPlayer)))[Ss.RL] == "admin") {
		room.sendAnnouncement("Não tens permissão para fazer um jogador Admin!", byPlayer.id);
